        ...
        loop {
            ...
            for event in events.iter() {
                if event.token() == MainController::KEYBOARD_CHECK_TOKEN {
                    ...
                    let (services_request, services): (
                        Vec<&DiscoveredTopicData>,
                        Vec<&DiscoveredTopicData>,
                    ) = services
                        .iter()
                        .partition(|p| p.topic_name().starts_with("rq"));
                    let (services_reply, dds_topics): (
                        Vec<&DiscoveredTopicData>,
                        Vec<&DiscoveredTopicData>,
                    ) = services
                        .iter()
                    ...
                    for (i, topic_info) in topics.iter().enumerate() {
                        let ft =
                            format!("{} - {}", topic_info.topic_name(), topic_info.type_name());
                    ...
                    for (i, reply_info) in services_reply.iter().enumerate() {
                        let ft =
                            format!("{} - {}", reply_info.topic_name(), reply_info.type_name());
                        max_width = if ft.len() > max_width {
                            ft.len()
                        } else {
                            max_width
