...
pub fn create_paragraph_from_string_list(strings : Vec<String>, title :String) ->  Paragraph<'static> {
  create_paragraph_form_spans_list(create_spans_list_from_string_list(strings), title)
}

fn create_paragraph_form_spans_list(spans : Vec<Spans<'static>> , title : String) ->  Paragraph<'static> {
  Paragraph::new(spans)
  .block(Block::default().title(title).borders(Borders::ALL))
