...
impl<'a> VisualizatorApp<'a> {
  ...
  pub fn ui<B: Backend>(&mut self, f: &mut Frame<B>) {  
    ...
    let selected_topic_paragraph = create_paragraph_from_string_list( self.get_selected_topic_strings(),"Topic information".to_string());
    let selected_participant_paragraph = create_paragraph_from_string_list(self.get_selected_participant_strings(),"Participant information".to_string());
    let selected_local_node_paragraph = create_paragraph_from_string_list(self.get_selected_local_node_strings(),"Node information".to_string());
    let _selected_external_node_paragraph = create_paragraph_from_string_list(self.get_selected_external_node_strings(),"Node information".to_string());


    match self.tab_index{
      0 => {
        f.render_stateful_widget(list_of_participants, first_row[0], &mut self.ros_participants_display_items.state);
        f.render_widget(selected_participant_paragraph, first_row[1]);
      },
      1 => {
        f.render_stateful_widget(list_of_local_nodes, first_row[0], &mut self.local_nodes_display_items.state);
      ...
      */
      2 => {
        f.render_stateful_widget(list_of_topics, first_row[0], &mut self.topic_list_display_items.state);
