...
use messages::RosCommand;
use visualizator_app::VisualizatorApp;
use messages::DataUpdate;
  ...
  loop{
    ...
    for event in events.iter() {
      if event.token() == ROS2_NODE_RECEIVED_TOKEN {
        ...
        let mut node_infos : Vec<NodeInfo> = ros_participant.get_all_discovered_local_ros_node_infos().values().map(|x|x.clone()).collect();
        let node_infos_external_vec : Vec<Vec<NodeInfo>> = ros_participant.get_all_discovered_external_ros_node_infos().values().map(|x|x.clone()).collect();
