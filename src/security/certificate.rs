...
use crate::security::{
  ...
  config::{to_config_error_parse, ConfigError, to_config_error_other, },
...
impl Certificate {
  ...
  pub fn subject_name_der(&self) -> Result<Vec<u8>,ConfigError> {
    ...
    er.write_encoded(Mode::Der, &mut buf)
      .map_err(to_config_error_other("Cannot extract subjcet_name DER encoding"))?;
    Ok(buf)
  }
  ...
  pub fn verify_signed_by_certificate( &self, other: &Certificate) -> SecurityResult<()> {
    self.cert.verify_signed_by_certificate(&other.cert)
      .map_err(|e| security_error(&format!("Certificate signature verification failure: {e:?}")))
  }
