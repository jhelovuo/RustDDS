...
const QOS_PRIVATE_KEY_PROPERTY_NAME: &str = "dds.sec.auth.privte_key";
...
impl Authentication for AuthenticationBuiltin {
  ...
  fn validate_local_identity(
    ...
    // * identity_ca - This is the auhtority that verifies the authenticity of
    ...
    // certificate to other Particiapnts, they can verify (with their copy of the CA certificate) that
    // the given public key and subject name belong together. The Domain Governance document
    // gives permissions to subject names, and this binding confirms that the permissions are
    // applicable to he holder of certain public-private-key pair holders.
    //
    ...
    // The CA certificate may have a revocation list and/or check an on-line OCSP server.)
    //
    // The returned IdentityHandle must be capable of
    // * reading this participant's public key (from identity_certificte)
    // * performing verify and sign operations with this participant's private key
    // * accessing the participan GUID (candidate or adjusted??)

    //TODO: These loading code snippets are too cut-and-paste. Copied from access control.
