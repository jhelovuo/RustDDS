...
impl Serialize for SubmessageFlag {
        ...
        where S: Serializer
    {
        ...
        if self.flags[0] { result |= 0b0000_0001 };
        if self.flags[1] { result |= 0b0000_0010 };
        if self.flags[2] { result |= 0b0000_0100 };
        if self.flags[3] { result |= 0b0000_1000 };
        if self.flags[4] { result |= 0b0001_0000 };
        if self.flags[5] { result |= 0b0010_0000 };
        if self.flags[6] { result |= 0b0100_0000 };
        if self.flags[7] { result |= 0b1000_0000 };
        ...
        where D: Deserializer<'de>
    {
        ...
        impl<'de> Visitor<'de> for SubmessageFlagVisitor {
            ...
            fn visit_u8<E>(self, v: u8) -> Result<Self::Value, E> {
                Ok(SubmessageFlag{ flags: [v & (1 << 0) != 0,
                                           v & (1 << 1) != 0,
                                           v & (1 << 2) != 0,
                                           v & (1 << 3) != 0,
                                           v & (1 << 4) != 0,
