...
/// The DataFrag Submessage extends the Data Submessage by enabling the serializedData to be
/// fragmented and sent as multiple DataFrag Submessages. The fragments contained in the DataFrag
/// Submessages are then re-assembled by the RTPS Reader.
...
pub struct DataFrag {
    ...
    /// the RTPS Writer identified by the writerGuid.
    ...
    /// Indicates the starting fragment for the series of fragments in serialized_data.
    ...
    /// Encapsulation of a consecutive series of fragments, starting at fragment_starting_num
    /// for a total of fragments_in_submessage. Represents part of the new value of the data-object
    /// after the change. Present only if either the DataFlag or the KeyFlag are set in the header.
    /// Present only if DataFlag is set in the header.
