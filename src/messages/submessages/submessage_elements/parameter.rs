...
impl Parameter {
    ...
    /// DataWriter has disposed the instance of the data-object whose Key
    /// appears in the submessage
    /// is_unregistered=1 indicates that the DDS DataWriter has unregistered
    /// the instance of the data-object whose Key appears in the submessage
    /// is_filtered=1 indicates that the DDS DataWriter has written as
    /// sample for the instance of the data-object whose Key appears in the
    /// submessage but the sample did not pass the content filter specified
    /// by the DDS DataReader. The status info parameter may appear in the
    /// Data or in the DataFrag submessages for additional info look https://www.omg.org/spec/DDSI-RTPS/2.3/PDF -> 9.6.3.9 StatusInfo_t (PID_STATUS_INFO)
