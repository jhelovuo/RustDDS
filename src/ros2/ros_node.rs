...
impl RosParticipantInner {
    ...
    pub fn from_domain_participant(
        ...
        let ros_discovery_publisher =
            domain_participant.create_publisher(&ROSDiscoveryTopic::qos())?;
...
impl NodeOptions {
    /// # Arguments
    ///
    /// * `enable_rosout` -  Wheter or not ros logging is enabled (rosout
    ///   writer)
...
impl RosNode {
    ...
    /// * `qos` - Quality of Service parameters for the topic (not restricted
    ///   only to ROS2)
    /// * `topic_kind` - Does the topic have a key (multiple DDS instances)?
    ///   NoKey or WithKey
    ///
    ///  
    ...
    ///
    /// * must not be empty
    /// * may contain alphanumeric characters ([0-9|a-z|A-Z]), underscores (_),
    ///   or forward slashes (/)
    /// * may use balanced curly braces ({}) for substitutions
    /// * may start with a tilde (~), the private namespace substitution
    ///   character
    /// * must not start with a numeric character ([0-9])
    /// * must not end with a forward slash (/)
    /// * must not contain any number of repeated forward slashes (/)
    /// * must not contain any number of repeated underscores (_)
    /// * must separate a tilde (~) from the rest of the name with a forward
    ///   slash (/), i.e. ~/foo not ~foo
    ...
    /// * `qos` - Should take [QOS](../dds/qos/struct.QosPolicies.html) and use
    ...
    /// Creates ROS2 Subscriber to [Keyed](../dds/traits/trait.Keyed.html)
    /// topic.
    ///
    /// # Arguments
    ///
    /// * `topic` - Reference to topic created with `create_ros_topic`.
    /// * `qos` - Should take [QOS](../dds/qos/struct.QosPolicies.html) and use
    ///   it if it's compatible with topics QOS. `None` indicates the use of
    ///   Topics QOS.
