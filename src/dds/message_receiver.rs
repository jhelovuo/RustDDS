...
pub(crate) struct MessageReceiver {
    ...
    // GuidPrefix sent in this channel needs to be RTPSMessage source_guid_prefix. Writer needs
    // this to locate RTPSReaderProxy if negative acknack.
            ...
            EntitySubmessage::Data(data, data_flags) => {
                ...
                if data.reader_id == EntityId::UNKNOWN {
                    ...
                    for reader in self
                        .available_readers
                        .values_mut()
                        // exception: discovery prococol reader must read from unkonwn discovery
                        // protocol writers TODO: This logic here is
                        // uglyish. Can we just inject a presupposed writer
                        // (proxy) to the built-in reader as it is created?
                        .filter(|r| {
            ...
            EntitySubmessage::AckNack(acknack, _) => {
                    ...
                    .try_send((self.source_guid_prefix, AckSubmessage::AckNack(acknack)))
                {
                    ...
                    Err(TrySendError::Full(_)) => {
                        info!(
                            "AckNack pipe full. Looks like I am very busy. Discarding submessage."
                        )
                    }
        ...
        match interp_subm {
            ...
            InterpreterSubmessage::InfoReply(info_reply, flags) => {
                ...
                if flags.contains(INFOREPLY_Flags::Multicast) {
                    self.multicast_reply_locator_list = info_reply.multicast_locator_list.expect(
                        "InfoReply flag indicates multicast locator is present but none found.",
                    );
...
mod tests {
    ...
    fn test_shapes_demo_message_deserialization() {
        ...
        let writer_ing = WriterIngredients {
            guid: GUID::new_with_prefix_and_id(
                gui_prefix,
                EntityId::create_custom_entity_id(
                    [0, 0, 2],
    ...
    fn mr_test_submsg_count() {
        ...
        let udp_bits1 = Bytes::from_static(&[
            ,
        ]);
        ...
        let udp_bits2 = Bytes::from_static(&[
            0x52, 0x54, 0x50, 0x53, 0x02, 0x03, 0x01, 0x0f, 0x01, 0x0f, 0x99, 0x06, 0x78, 0x34,
            0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x0e, 0x01, 0x0c, 0x00, 0x01, 0x03, 0x00, 0x0c,
            0x29, 0x2d, 0x31, 0xa2, 0x28, 0x20, 0x02, 0x08, 0x06, 0x03, 0x18, 0x00, 0x00, 0x00,
            0x04, 0xc7, 0x00, 0x00, 0x04, 0xc2, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00,
