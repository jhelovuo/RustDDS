...
/// as it allocates network sockets, creates background threads, and allocates some memory
/// for object caches.
///
...
/// Domains are identified by a domain identifier, which is, in Rust terms, a `u16`.
/// Domain identifer values are application-specific, but `0` is usually the default. 
#[derive(Clone)]
