...
impl Reader {
                        ...
                        Some(last_change) => {
                            ...
                            if since_last > deadline_duration {
                                ...
                                changes.push(DataReaderStatus::RequestedDeadlineMissed {
                                    count: CountWithChange::start_from(
                                        self.requested_deadline_missed_count,
                                        1,
                                    ),
                                });
                            }
    ...
    pub fn handle_datafrag_msg(
        ...
        let writer_guid =
            GUID::new_with_prefix_and_id(mr_state.source_guid_prefix, datafrag.writer_id);
            ...
            if let Some(writer_proxy) = self.matched_writer_lookup(writer_guid) {
                if writer_proxy.contains_change(writer_sn) {
                    ...
                    if my_entityid == EntityId::SPDP_BUILTIN_PARTICIPANT_READER {
                        ...
                        // This is an attmpted workaround to eProsima FastRTPS
                        // not incrementing sequence
                        // numbers. (eProsime shapes demo 2.1.0 from
                        // 2021)
                    } else {
                        return;
                    }
            ...
            (None, false, false) => {
                    ...
                    .flatten()
                {
                    ...
                    None => {
                        ...
                        // The coherent set end marker is sent as DATA with no payload and not key,
                        // only Inline QoS.
                ...
                match missing_seqnums.get(0) {
                    Some(&first_missing) => {
                        ...
                        SequenceNumberSet::from_base_and_set(
                            first_missing,
                            &BTreeSet::from_iter(missing_seqnums.iter().copied().take_while(
                                |sn| sn < &(first_missing + SequenceNumber::from(256)),
                            )),
                        )
                    }
    ...
    pub fn notify_cache_change(&self) {
        match self.notification_sender.try_send(()) {
            ...
            Err(mio_channel::TrySendError::Disconnected(_)) => {
                // If we get here, our DataReader has died. The Reader should
                // now dispose itself. TODO: Implement Reader
                // disposal.
            }
