...
use crate::{
    dds::{
        ...
        values::result::*,
        with_key::{
            datareader::DataReader as WithKeyDataReader,
            datawriter::DataWriter as WithKeyDataWriter,
        },
...
impl Publisher {
    ...
    /// Creates DDS [DataWriter](struct.With_Key_DataWriter.html) for Keyed
    /// topic
    ///
    /// # Arguments
    ///
...
impl Subscriber {
    ...
    where
        D: DeserializeOwned,
    {
        self.create_datareader_no_key_with_entityid::<D, CDRDeserializerAdapter<D>>(
            topic, entity_id, qos,
        )
    }
        ...
        let dp = match self.participant() {
            ...
            None => {
                return log_and_err_precondition_not_met!(
                    "DomainParticipant doesn't exist anymore."
                )
            }
        };
