...
///
/// They can act as a domain of sample ordering or atomicity, if such QoS policies
/// are used. For example, DDS participants could agree via QoS policies that
/// data samples must be presented to readers in the same order as writers have
/// written them, and the ordering applies also between several writers/readers,
/// but within one publisher/subscriber. Analogous arrangement can be set up
/// w.r.t. coherency: All the samples in a transaction are delivered
/// to the readers, or none are. The transaction can span several readers, writers,
/// and topics in a single publisher/subscriber.
///
///
/// # Examples
///
