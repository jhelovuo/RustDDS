  ...
  DA: DeserializerAdapter<D> + 'static,
{
  ...
  pub fn as_async_stream(&self) -> impl Stream< Item=Result<DeserializedCacheChange<D>> > + '_ {
    self.keyed_simpledatareader.as_async_stream()
  ...
  pub fn as_simple_data_reader_event_stream(&self) 
    -> impl Stream< Item=std::result::Result<DataReaderStatus, std::sync::mpsc::RecvError> > + '_
  {
    self.keyed_simpledatareader.as_simple_data_reader_event_stream()
  }
