...
impl Writer {
    ...
    pub fn notify_new_data_to_all_readers(&mut self) {
        ...
        //   reader_proxy.notify_new_cache_change(self.
    ...
    /// This is called by dp_wrapper everytime cacheCleaning message is
    /// received.
    ...
    /// When receiving an ACKNACK Message indicating a Reader is missing some
    /// data samples, the Writer must respond by either sending the missing
    /// data samples, sending a GAP message when the sample is not relevant,
    /// or sending a HEARTBEAT message when the sample is no longer
    /// available
            ...
            match self.sequence_number_to_instant(unsent_sn) {
                Some(timestamp) => {
                        ...
                        .from_topic_get_change(&self.my_topic_name, &timestamp)
                    {
                        ...
                        // TODO: Here we are cloning the entire payload. We need
                        // to rewrite the transmit path
                        // to avoid copying.
    ...
    /// CacheChanges can be safely removed only if they are acked by all
    /// readers. (Reliable) Depth is QoS policy History depth.
