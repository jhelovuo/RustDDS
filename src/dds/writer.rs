...
use crate::{submessages::{Heartbeat, InfoSource, SubmessageHeader, SubmessageKind, SubmessageFlag, InterpreterSubmessage, EntitySubmessage}, structure::cache_change::CacheChange, serialization::{SubMessage, Message}};
...
impl Writer {
  pub fn new(participant_guid_prefix: GuidPrefix, writer_id : EntityId) -> Writer {
    Writer {
  ...
  fn create_submessage_header_flags(&self, kind : &SubmessageKind) -> SubmessageFlag{
    let mut sub_flags: SubmessageFlag =  SubmessageFlag{flags: 0b0000000_u8};
  ...
  fn create_message_header(&self) -> Header{
    let head : Header = Header{
      ...
      protocol_version:  ProtocolVersion{ major : self.source_version.major, minor : self.source_version.minor },
      vendor_id: VendorId {vendorId : self.source_vendor_id.vendorId} ,
      guid_prefix: self.source_guid_prefix,
    };
    return head;
  }

  fn create_submessage_header(&self, kind : SubmessageKind, submessageLength :u16) -> SubmessageHeader{
   ...
   pub fn get_TS_submessage(&self) -> SubMessage{
    ...
    let s: SubMessage = SubMessage{
      ...
      submessage : None,
      intepreterSubmessage : Some(InterpreterSubmessage::InfoTimestamp(timestamp,self.create_submessage_header_flags(&SubmessageKind::INFO_TS))),
    };
    return s
  }
  ...
  pub fn get_DATA_msg_from_cache_change(& self, change : CacheChange) -> SubMessage{
    ...
    data_message.serialized_payload.representation_identifier = LittleEndian::read_u16(&representationIdentifierBytes);
    ...
    data_message.serialized_payload.value = change.data_value.unwrap().serialized_payload.value;

    let size = data_message.write_to_vec_with_ctx(self.endianness).unwrap().len() as u16;
    let s: SubMessage = SubMessage{
      header : self.create_submessage_header(SubmessageKind::DATA, size),
      submessage : Some(crate::submessages::EntitySubmessage::Data(data_message,self.create_submessage_header_flags(&SubmessageKind::DATA))),
  ...
  pub fn get_heartbeat_msg(&mut self) -> SubMessage{
    ...
    let s: SubMessage = SubMessage {
      header : head,
      intepreterSubmessage : None,
      submessage : Some(EntitySubmessage::Heartbeat(heartbeat, self.create_submessage_header_flags(&SubmessageKind::HEARTBEAT))),
    };
    return s;
  ...
  pub fn write_user_msg(&mut self, change : CacheChange) -> Vec<u8>{
    ...
    let size = destiny.write_to_vec_with_ctx(self.endianness).unwrap().len();
