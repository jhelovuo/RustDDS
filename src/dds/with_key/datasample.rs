...
/// A data sample and its associated [metadata](`SampleInfo`) received from a WITH_KEY Topic.
///
/// Note that [`no_key::DataSample`](crate::no_key::DataSample) and 
/// [`with_key::DataSample`](crate::with_key::DataSample) are two different structs.
///
...
/// * `Err(k)` means `valid_data == false`, no sample exists, but only a Key `k` and
