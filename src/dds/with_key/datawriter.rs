...
use crate::{
    dds::{
        ...
        topic::Topic,
        traits::{
            dds_entity::DDSEntity, key::*, serde_adapters::with_key::SerializerAdapter,
            TopicDescription,
        },
        values::result::{Error, Result},
    },
    discovery::{
        data_types::topic_data::SubscriptionBuiltinTopicData, discovery::DiscoveryCommand,
    },
    ...
    SA: SerializerAdapter<D>,
{
    ...
    /// by the reliable DataWriter entities is acknowledged by all
        ...
        match &self.qos_policy.reliability {
            ...
            Some(Reliability::Reliable { .. }) => {
                ...
                if let Some(_event) = events.iter().next() {
                    let _ = acked_receiver.try_recv().or_else(|_e| {
                        log_and_err_internal!("wait_for_acknowledgments - Spurious poll event?")
                    });
                    // got reply
                    Ok(true)
                } else {
    ...
    */
    ...
    /// Manually asserts liveliness (use this instead of refresh) according to
    /// QoS
    ///
    /// # Examples
    ///
