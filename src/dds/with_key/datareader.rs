    ...
    DA: DeserializerAdapter<D>,
{
    ...
    /// This is not called `iter()` because it takes a mutable reference to
    /// self.
    ///
    /// # Examples
    ///
    ...
    /// <strong>Note!</strong> If the iterator is only partially consumed, all
    /// the samples it could have provided are still removed from the
    /// `Datareader`.
    ///
    /// # Examples
    ///
                ...
                match data_value {
                    DDSData::Data { serialized_payload } => {
                            ...
                            .find(|r| **r == serialized_payload.representation_identifier)
                        {
                            ...
                            continue; // skip this sample, as we cannot decode
                                      // it
                        }
                    }
                    ...
                    DDSData::DisposeByKey {
                        ...
                        match DA::key_from_bytes(
                            ...
                            Ok(key) => self.datasample_cache.add_sample(
                                Err(key),
                                *writer_guid,
                                instant,
                                *source_timestamp,
...
mod tests {
    ...
    fn dr_get_samples_from_ddschache() {
        ...
        let deserialized_random_data = matching_datareader.read(1, ReadCondition::any()).unwrap()
            [0]
        .value()
        .unwrap()
        .clone();
