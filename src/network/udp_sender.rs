...
impl UDPSender {
  pub fn new(sender_port: u16) -> io::Result<UDPSender> {
    let saddr: SocketAddr = if current_OS == "windows" {
      SocketAddr::new("".parse().unwrap(), sender_port)
    } else {
      SocketAddr::new("0.0.0.0".parse().unwrap(), sender_port)
    };
  ...
  pub fn send_to_locator(&self, buffer: &[u8], l: &Locator) {
    match l.kind {
      LocatorKind::LOCATOR_KIND_UDPv4 | LocatorKind::LOCATOR_KIND_UDPv6 => {
