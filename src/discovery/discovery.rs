...
use crate::{
    dds::{
        ...
        qos::{
            policy::{
                Deadline, DestinationOrder, Durability, History, Liveliness, Ownership,
                Presentation, PresentationAccessScope, Reliability, TimeBasedFilter,
            },
    ...
    discovery::{
        data_types::{
            ...
            topic_data::{
                ...
                DiscoveredWriterData, DiscoveredWriterDataKey, PublicationBuiltinTopicData,
...
pub(crate) struct Discovery {
    ...
    dcps_topic_reader:
        DataReader<DiscoveredTopicData, PlCdrDeserializerAdapter<DiscoveredTopicData>>,
            ...
            for event in events.into_iter() {
                match event.token() {
                        ...
                        while let Ok(command) = self.discovery_command_receiver.try_recv() {
                            match command {
                                DiscoveryCommand::StopDiscovery => {
                                    ...
                                    for writer in db.get_all_local_topic_writers() {
                                        self.dcps_publication_writer
                                            .dispose(
                                                DiscoveredWriterDataKey(
                                                    writer.writer_proxy.remote_writer_guid,
                                                ),
                                                None,
                                            )
                                            .unwrap_or(());
                                    ...
                                    self.dcps_participant_writer
                                        .dispose(
                                            SpdpDiscoveredParticipantDataKey(
                                                self.domain_participant.guid(),
                                            ),
                                            None,
                                        )
                                        .unwrap_or(());
                                ...
                                DiscoveryCommand::ManualAssertLiveliness => {
                                    self.liveliness_state.last_manual_participant_update =
                                        Timestamp::now();
                                }
                    ...
                    DISCOVERY_PARTICIPANT_CLEANUP_TOKEN => {
                        ...
                        self.participant_cleanup_timer
                    ...
                    DISCOVERY_SEND_PARTICIPANT_INFO_TOKEN => {
                        let strong_dp = match self.domain_participant.clone().upgrade() {
                            ...
                            None => {
                                error!(
                                    "DomainParticipant doesn't exist anymore, exiting Discovery."
                                );
                                return;
                            }
                        };
                        ...
                        // reschedule timer
                        self.participant_send_info_timer
                    ...
                    DISCOVERY_PARTICIPANT_MESSAGE_TIMER_TOKEN => {
                        ...
                        self.dcps_participant_message_timer
                ...
                Ok(Some(d)) => match d.value {
                    Ok(participant_data) => {
                        let participant_data = participant_data.clone(); // .clone() is necessary, because .read
                                                                         // returns references to within Reader, so we cannot operate on self until
                                                                         // we
                                                                         // clone.
                        debug!(
                    ...
                    Err(guid) => {
                        ...
                        self.send_discovery_notification(
                            DiscoveryNotificationType::ParticipantLost {
        ...
        for d in dwds {
            match d {
                Ok(dwd) => {
                    ...
                    {
                        self.send_discovery_notification(
                            DiscoveryNotificationType::WriterUpdated {
        ...
        for t in ts {
            match t {
                Ok(topic_data) => {
                    ...
                    if updated {
                        self.send_discovery_notification(
                            DiscoveryNotificationType::TopicsInfoUpdated,
                        );
                    }
                }
                // Err means disposed
...
mod tests {
    ...
    use crate::{
        dds::{
            participant::DomainParticipant, traits::serde_adapters::no_key::DeserializerAdapter,
        },
    ...
    fn discovery_reader_data_test() {
        ...
        let _writer = publisher
            .create_datawriter_cdr::<ShapeType>(topic.clone(), None)
            .unwrap();
                ...
                SubmessageBody::Entity(v) => {
                    match v {
                        EntitySubmessage::Data(d, _) => {
                            ...
                            drd.reader_proxy.unicast_locator_list.push(Locator::from(
                                SocketAddr::new("127.0.0.1".parse().unwrap(), 11001),
                            ));
                            drd.reader_proxy.multicast_locator_list.clear();

                            data = Bytes::from(
                                to_bytes::<DiscoveredReaderData, byteorder::LittleEndian>(&drd)
                                    .unwrap(),
                            );
