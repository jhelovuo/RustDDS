...
use crate::{
    dds::{
        ...
        qos::{
            policy::{
                Deadline, DestinationOrder, Durability, History, LatencyBudget, Lifespan,
                Liveliness, Ownership, Presentation, Reliability, ResourceLimits, TimeBasedFilter,
            },
...
impl<'de> Deserialize<'de> for SubscriptionBuiltinTopicData {
        ...
        D: serde::Deserializer<'de>,
    {
        let custom_ds = BuiltinDataDeserializer::new();
        let res = deserializer.deserialize_any(custom_ds)?;
        res.generate_subscription_topic_data()
...
impl<'de> Deserialize<'de> for DiscoveredReaderData {
        ...
        D: serde::Deserializer<'de>,
    {
        let custom_ds = BuiltinDataDeserializer::new();
        let res = deserializer.deserialize_any(custom_ds)?;
        res.generate_discovered_reader_data()
            .map_err(serde::de::Error::custom)
    }
}
...
impl<'de> Deserialize<'de> for DiscoveredReaderDataKey {
        ...
        D: serde::Deserializer<'de>,
    {
        let custom_ds = BuiltinDataDeserializer::new();
        let res = deserializer.deserialize_any(custom_ds)?;
        res.generate_discovered_reader_data_key()
            .map_err(serde::de::Error::custom)
    }
}
...
impl<'de> Deserialize<'de> for PublicationBuiltinTopicData {
        ...
        D: serde::Deserializer<'de>,
    {
        let custom_ds = BuiltinDataDeserializer::new();
        let res = deserializer.deserialize_any(custom_ds)?;
        res.generate_publication_topic_data()
...
impl ParticipantMessageDataKind {
    ...
    pub const PARTICIPANT_MESSAGE_DATA_KIND_AUTOMATIC_LIVELINESS_UPDATE:
        ParticipantMessageDataKind = ParticipantMessageDataKind {
...
mod tests {
    ...
    fn td_subscription_builtin_topic_data_ser_deser() {
        ...
        let sdata =
            to_bytes::<SubscriptionBuiltinTopicData, LittleEndian>(&sub_topic_data).unwrap();
        let sub_topic_data2: SubscriptionBuiltinTopicData =
            PlCdrDeserializerAdapter::from_bytes(&sdata, RepresentationIdentifier::PL_CDR_LE)
                .unwrap();
        assert_eq!(sub_topic_data, sub_topic_data2);
        let sdata2 =
            to_bytes::<SubscriptionBuiltinTopicData, LittleEndian>(&sub_topic_data2).unwrap();
        assert_eq!(sdata, sdata2);
    }
    ...
    fn td_publication_builtin_topic_data_ser_deser() {
        ...
        let sdata2 =
            to_bytes::<PublicationBuiltinTopicData, LittleEndian>(&pub_topic_data2).unwrap();
        assert_eq!(sdata, sdata2);
    }
    ...
    fn td_discovered_reader_data_ser_deser() {
        ...
        let raw_data = Bytes::from_static(&[
            ,
