...
// TODO: How to store the socket so that it is correctly tracked and dropped when
// these PolEventSource /-Sender are dropped. Can we do that without unsafe?
...
pub fn make_poll_channel() -> io::Result<(PollEventSource, PollEventSender)> {
  // This could be also a pipe or any other OS construct that can be poll()'ed in mio-0.8
  // And can be triggered (sent to) from the application.
  // Socketpair is symmetric, but we decide to call one end receive and the other send.
  // We use stream sockets, because the documentation says datagram sockets are not
  // available on MacOS.
  ...
  Ok(
    ( PollEventSource { rec_sps,  rec_mio_socket: Mutex::new(rec_mio_socket) , },
      PollEventSender { send_sps, send_mio_socket: Mutex::new(send_mio_socket),  },
    ) 
    )
}
...
impl event::Source for PollEventSource
{
  ...
  {
    self.rec_mio_socket.lock().unwrap().register(registry,token,interests)
  }

  fn reregister(&mut self, registry: &Registry, token: Token, interests: Interest) -> io::Result<()>
  {
    self.rec_mio_socket.lock().unwrap().reregister(registry, token, interests)
  }
